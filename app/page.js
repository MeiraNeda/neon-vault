    ...
    <>
      ...
      <div className="overflow-x-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 py-28">
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold text-center mb-20 tracking-tight neon-text">
            Pourquoi entrer dans le{' '}
            ...
            {[
              {
                ...
                desc: 'Des ressources que personne d’autre ne possède. Vault fermé au monde extérieur.',
              },
              {
                icon: '🔐',
                title: 'Accès Sécurisé',
                desc: 'Authentification biométrique simulée. Seul l’utilisateur connecté peut lire.',
              },
              {
                icon: '⚡',
                title: 'Transaction Instantanée',
                desc: 'Paiement Gumroad en un éclair. PDF livré directement dans ton espace.',
              },
            ].map((item, i) => (
              <div
                key={i}
                className="glass-neon rounded-3xl p-8 sm:p-12 card-cyber text-center"
              >
                <div className="text-5xl sm:text-6xl mb-6 sm:mb-8">{item.icon}</div>
                <h3 className="text-2xl sm:text-3xl font-semibold mb-5 tracking-tight">
                  {item.title}
                </h3>
                <p className="text-zinc-400 text-base sm:text-lg">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
